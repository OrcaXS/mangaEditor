...
export default {
    ...
    generateCharConfig() {
      ...
      for (let i = 0; i < this.textConfig.text.length; i += 1) {
        ...
        if (char !== '\n') {
          ...
          console.log({ offsetX, offsetY, leftOffset, topOffset });
        }
      }
  ...
  render(createElement) {
    ...
    Object.entries(this.textAreas).forEach(([idx, val]) => {
      ...
      for (let i = 0; i < textContent.length; i += 1) {
        ...
        if (char !== '\n') {
          ...
          textArr.push(createElement('vText', {
            props: {
              config: {
                x: this.x + offsetX + leftOffset + (this.fontSize / 2),
                y: this.y + offsetY + topOffset + (this.fontSize / 2),
    ...
    const textGroupVNode = createElement('vGroup', vGroupArr);
